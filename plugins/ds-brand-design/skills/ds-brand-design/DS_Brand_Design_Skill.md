...
- **Newsletter field hover (2026-09-22, Chris Provins).** The footer sign-up (pill email input with an Eucalyptus arrow button drawn as an image) had focus and active states but no hover. Added, scoped to the footer form: hovering the field deepens the stroke from Border/Brand Eucalyptus to Eucalyptus Dark `#39381B`, lifts the surface from transparent to Neutral 100 `#F9F9F5` (Default 200ms / Standard), and a **blinking underscore appears after the placeholder**, a caret waiting for input, 0.8s step blink. Two richer versions were tried and cut the same day: the arrow leaning forward 3 to 6px, then the placeholder erasing and retyping character by character. Chris kept only the caret. **Rules that fell out:** an input's hover is a lighter version of its focus, never a different colour; a field's hover should suggest typing, not perform it — one blinking caret says "this is where you type" and anything more is theatre; the effect runs only for hover-capable pointers and never under reduced motion; and the overlay copies the input's computed font and `::placeholder` colour at runtime so it is indistinguishable from the real placeholder.
...
```json
{
  ...
  "last_skill_sync": "2026-09-22T18:42:52.438431+00:00",
