...
- **Pre-footer CTA on DS/Pattern/Camo with footer-zone parallax.** The inherited template used a flat exported camo JPEG as the section background. Replaced with the canonical generator output (`generateCamoSvg(2026, { width: 1920, height: 720 })`) inlined as an absolutely positioned layer, 30% taller than the section, translating −15% → +15% across the section's pass through the viewport — the Parallax pattern's cap, applied to the pre-footer band (the "footer" zone). Scroll-driven (`animation-timeline: view(block)`, `animation-range: cover`) where supported, `requestAnimationFrame` fallback elsewhere so the effect is universal, switched off under reduced motion. Content keeps the existing chartreuse card, so the chartreuse-on-camo type rule is honoured by the card rather than by recolouring type over the pattern. **Gotcha, second sighting:** clipping the section with `overflow: hidden` silently killed the parallax — any `overflow` other than `visible`/`clip` makes that element a scroll container, and `animation-timeline: view()` binds to the nearest one, which then never scrolls. Clip parallax layers with `clip-path: inset(0)` (or `overflow: clip`), never `overflow: hidden`. Same root cause as the body-overflow note above; it applies at every level of nesting.
...
```json
{
  ...
  "last_skill_sync": "2026-09-17T20:07:51.465200+00:00",
