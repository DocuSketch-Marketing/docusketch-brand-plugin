...
- **Newsletter field hover (2026-09-22, Chris Provins).** The footer sign-up (pill email input with an Eucalyptus arrow button drawn as an image) had focus and active states but no hover. Added, scoped to the footer form: hovering the field deepens the stroke from Border/Brand Eucalyptus to Eucalyptus Dark `#39381B`, lifts the surface from transparent to Neutral 100 `#F9F9F5` (Default 200ms / Standard), and **erases the placeholder one character at a time behind a blinking underscore** (22ms per character, 14ms retyping; a first cut at 45 / 28 read as hesitant and Chris asked for faster), as if someone were clearing it to type; it types back in when the pointer leaves, and stops the moment the field is focused or holds a value. A first pass also leaned the arrow forward 3 to 6px; Chris cut it — the erase is the whole idea, and a second moving part competes with it. **Rules that fell out:** an input's hover is a lighter version of its focus, never a different colour; one hover, one idea — when a field's hover tells a story (the caret), nothing else on the control moves; the effect runs only for hover-capable pointers and never under reduced motion; and the overlay copies the input's computed font and `::placeholder` colour at runtime so it is indistinguishable from the real placeholder.
...
```json
{
  ...
  "last_skill_sync": "2026-09-22T18:27:47.659651+00:00",
