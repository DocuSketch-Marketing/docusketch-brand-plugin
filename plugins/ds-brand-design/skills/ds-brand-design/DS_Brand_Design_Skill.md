...
**Type over Camo — colour follows the seed (revised 2026-09-18).** The original rule was chartreuse only. The first shipped application (the Capture page workflow panel, Chris Provins) showed that Chartreuse 300 type on a bright camo field reads as one flat colour and looks off-brand, so the rule now has two states keyed to the camo's composition:
- **Dark-seeded camo** (blobs include Chartreuse 700/800/900 — the CTA band, most generator output): type is `DS/Color/Chartreuse 300` (`#E5DF00`). It advances off the dark blobs and reinforces the brand colour. Brand/Black disappears here.
- **Bright-seeded camo** (no 800/900 blobs, field reads yellow — pick the seed with the generator's brightness score): type is `Brand/Black` (`#1A1905`), hairlines `rgba(26,25,5,0.35)`, secondary copy at 80% opacity. This is the same pairing as Brand/Black on a Chartreuse 300 surface, so the panel reads as a brand surface rather than a pattern with a caption.
- **Dark-base camo** (`DS/Pattern/Camo/Dark`, added 2026-09-18 for the Capture page workflow panel: base `Chartreuse 900` `#2A2808`, blobs weighted to Chartreuse 800/900 and Brand/Ash with two or three low-alpha Chartreuse 300/600 accents): type is the light neutrals — `Neutral 100` `#F9F9F5` for headlines, labels and arrows, `Neutral 300` `#DFDDC8` for body and secondary copy, hairlines `rgba(249,249,245,0.3)`. The current-step or primary action flips to solid Chartreuse 300 with Brand/Black text. This is the statement-panel treatment: it reads as a dark brand surface with the pattern breathing underneath, not as a caption on a pattern.
Never mix two type colours on one camo surface beyond the single chartreuse accent, and never use pure white, Plex grey or a non-brand neutral — the neutrals above are the brand's own. Hover/active button fills still flip to solid Chartreuse 300 with Brand/Black text. If type will not read on a given seed, change the seed before changing the colour.
...
- **Workflow value panel on animated Camo (2026-09-18).** The cross-page "Capture / Scope / Estimate" module became a single rounded panel (`--radius-2xl`, 24) on `DS/Pattern/Camo` in its Animate mode — the same `generateCamoSvg` output with the kit's `pattern-blob` drift (22s ease-in-out, 2.2s stagger, off under reduced motion), a new seed per surface so two camo panels on one page never match. Left: eyebrow + one-sentence statement. Right: three hairline-divided rows, title + ↗ (or a "You are here" pill on the current step) + one-line description drawn from the positioning doc's workflow pillars. **Type on the panel is Chartreuse 300 on the standard generator camo** (seed 2027, hairlines at 50% alpha, secondary copy at 85%, outlined "You are here" pill) — the kit's original pairing. Chris tried three departures in one session — Brand/Black on a bright seed, light neutrals on a dark-base variant, chartreuse on that dark base — and rolled every one of them back to the canonical treatment. The rule's other two states stay documented as options that were built and can be reached for, but the canonical output stands as the default and the Capture page ships on it. An earlier pass used Olive on Neutral 100 cards and read as three product tiles rather than one platform statement. Row states: hover nudges the row `translateX(4px)` (kit list-row rule), turns its top hairline from 35% to solid Brand/Black, brings the description from 80% to full opacity and sends the arrow a further 6px; `:active` settles the row at 2px and the arrow at 3px; `:focus-visible` is a 2px Brand/Black outline at 4px offset. The current step has no link and carries a filled pill — Brand/Black with Chartreuse 300 text, the canonical primary fill — instead of an arrow. The arrow is the same 16px `→` SVG the testimonial slider arrows use, in `currentColor`, so one glyph does all directional work on the page. All at Default 200ms / Standard. Standing rule for this module: a Camo panel is a statement surface — one idea, one palette, no illustrations, no imagery fighting the pattern.
...
```json
{
  ...
  "last_skill_sync": "2026-09-18T20:48:47.796439+00:00",
