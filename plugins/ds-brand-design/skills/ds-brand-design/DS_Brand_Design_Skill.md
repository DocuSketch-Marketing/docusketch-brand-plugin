...
- **Stats band with a chartreuse glow (2026-09-18).** The business-impact band moved from three free-standing tiles to a Stripe-style proof row: centred condensed eyebrow + two-line H2, then four stats framed by Neutral 300 hairlines top and bottom (no vertical dividers), the lead stat in Text/Primary and the rest in Text/Secondary so one number carries the row, each with a one-line context sentence in body size (not caps). The glow beneath is a translation of the Figma frame `guoAdcJQH5m7hrlyH62OGZ` node `25:189`, which stacks three ellipses (Chartreuse 300 core, `#FFFA37` at 75% then 25%) under a 25% white frosted layer. In CSS that became one `radial-gradient(ellipse at 50% 100%, #E5DF00 0%, rgba(255,250,55,.75) 32%, rgba(255,250,55,.25) 62%, transparent 100%)` on an absolutely positioned layer 140% wide, anchored below the row, `filter: blur(48px)`, opacity 0.9. **Rules that fell out:** the glow is a surface, not a reveal — it does not animate, and it never sits behind text that has to stay readable at Text/Secondary contrast, which is why the row sits above it on `z-index` and the ellipse rises from below the hairline. Clip it with `overflow: clip` on the section (never `overflow: hidden`, which would kill the scroll-driven timelines elsewhere on the page). `#FFFA37` is a highlight mix from the Figma frame, not a token; use it only inside a glow that resolves to Chartreuse 300 at its core.
...
```json
{
  ...
  "last_skill_sync": "2026-09-18T15:42:06.260847+00:00",
