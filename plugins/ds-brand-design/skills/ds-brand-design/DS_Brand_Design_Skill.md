...
- **Newsletter field hover (2026-09-22, Chris Provins).** The footer sign-up (pill email input with an Eucalyptus arrow button drawn as an image) had focus and active states but no hover. Added, scoped to the footer form: hovering anywhere on the field deepens the stroke from Border/Brand Eucalyptus to Eucalyptus Dark `#39381B` and lifts the surface from transparent to Neutral 100 `#F9F9F5`; the arrow leans 3px forward, 5px when the arrow itself is hovered, 6px at 85% opacity on press; it stays forward while the input has focus. Default 200ms / Standard on colour and `translate` only. Wrapped in `@media (hover: hover)` so touch devices never get a stuck hover. **Rules that fell out:** an input's hover is a lighter version of its focus, never a different colour — the two must read as one control warming up; and a submit affordance inside a field gets a two-step hover (field, then control) so it reads as its own target. Use the `translate` property for the nudge when the element's centring already lives on `transform`.
...
```json
{
  ...
  "last_skill_sync": "2026-09-22T16:57:31.502168+00:00",
