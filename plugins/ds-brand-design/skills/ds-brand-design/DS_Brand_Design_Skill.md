...
| **Scroll reveal** | `IntersectionObserver` adds `.is-revealed` to the element. CSS transitions `opacity: 0 → 1` + `transform: translateY(…)`. **Two tiers, by target size:** *section-level blocks* (containers, headline groups, hero children) use **Emphasis** duration, Out easing, **14px** travel, no base delay, and trigger at `threshold: 0.2` with `rootMargin: '0px 0px -10% 0px'` (earlier triggers finish below the fold and read as no motion); *small items inside an already-revealed block* (list rows, cards in a grid) may use Default duration and 8px. One-shot per element — unobserve after reveal. Stagger sits one tier above the grid default on marketing pages: Loose for hero children and card grids, Default for slides / FAQ rows (cap 8). `transition-delay: calc(var(--index) * <stagger>)`. Never put `overflow-x: hidden` on `body` — it makes body the scroll container and disables the continuous variant; use `overflow-x: clip` on `html`. Opt elements in from JS (e.g. `data-reveal`) so a no-JS page renders everything visible. **Continuous variant for card rows:** where a row of cards should glide in with the scroll instead of firing on a trigger, use a CSS scroll-driven animation (`animation-timeline: view(block)`, `animation-range: entry 5% entry 55%`, each card offset ~7% further), `linear` timing (the scroll is the easing), opacity + `translateY(20px → 0)`, wrapped in `@supports (animation-timeline: view())` with the observer reveal as the fallback and `animation: none` under reduced motion. **Chartreuse wash variant (marketing):** instead of fading the block from transparent, keep it at full opacity and lay a `::after` overlay of `Chartreuse 300` with `mix-blend-mode: multiply` over it (`inset: 0`, `pointer-events: none`, `border-radius: inherit`), and transition the *overlay's* opacity `1 → 0` on the same duration/delay as the rise. Content is never invisible, the accent reads as a passing highlight rather than a static element, and the animated properties are still only transform and opacity. Works with the continuous variant by driving the overlay's opacity from the same `view()` timeline. | Section-level reveals on long pages. Marketing surfaces especially. See *Real-world context* below for where the two-tier rule, the continuous variant and the wash came from. |
...
```json
{
  ...
  "last_skill_sync": "2026-09-17T19:22:41.159954+00:00",
